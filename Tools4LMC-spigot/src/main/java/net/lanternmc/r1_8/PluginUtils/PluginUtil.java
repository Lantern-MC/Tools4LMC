...
public class PluginUtil {
    ...
    public static boolean isIgnored(String plugin) {
        for (String name : Tools4LMCSpigot.getheart().getIgnoredPlugins()) {
    ...
    public static String load(String name) {
        ...
        if (!pluginFile.isFile()) {
            for (File f : pluginDir.listFiles()) {
                if (f.getName().endsWith(".jar")) {
                    try {
                        PluginDescriptionFile desc = Tools4LMCSpigot.getheart().getPluginLoader().getPluginDescription(f);
        ...
        try {
            ...
            e.printStackTrace();
            return "§c该文件不是有效的plugin";
        }

        target.onLoad();
        Bukkit.getPluginManager().enablePlugin(target);

        return target.getName() + "已加载并启用";
